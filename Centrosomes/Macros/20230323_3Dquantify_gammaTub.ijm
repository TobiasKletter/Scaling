root = "/Volumes/TOB_WD2/Microscopy/20210726_RNAi/" + "/";
rawDir = root + "Crop_isotropic_CVcrop" + "/";
binaryDir = root + "Binary_CentrosomesGammaTub" + "/";
montageDir = root + "Binary_CentrosomesGammaTub_Montages" + "/";
outDir = root + "GammaTub_Volumes" + "/";
intensityDir = root + "3D_Intensity_gammaTub" + "/";
...
for (i = 0; i < fileList.length; i++) 
	...
	run("Split Channels");
	selectWindow("C4-" + fileList[i]); ////////// CHECK
	run("Grays");
	rename("C3_raw");
	...
	saveAs("Results", intensityDir + file_naked_name + "_Pole_gTub.csv");
	run("Clear Results");
	...
	setResult("Cell_ID", 0, file_naked_name);
	updateResults();
	saveAs("Results", outDir + file_naked_name + "_centrosome_volume.csv");
