root = "/Volumes/TOB_WD2/Microscopy/20230403_Centrosome" + "/";
rawDir = root + "Crop_CVcrop" + "/";
binaryDir = root + "Binary_CentrosomesCDK5RAP2" + "/";
montageDir = root + "Binary_CentrosomesCDK5RAP2_Montages" + "/";
outDir = root + "CDK5RAP2_Volumes" + "/";
intensityDir = root + "3D_Intensity_CDK5RAP2" + "/";
...
for (i = 0; i < fileList.length; i++) 
	...
	saveAs("Results", intensityDir + file_naked_name + "_Pole_CDK5RAP2.csv");
	run("Clear Results");
	...
	setResult("Cell_ID", 0, file_naked_name);
	updateResults();
	saveAs("Results", outDir + file_naked_name + "_centrosomeCDK5RAP2_volume.csv");
