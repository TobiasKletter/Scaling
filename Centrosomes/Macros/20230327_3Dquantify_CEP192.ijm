root = "/Volumes/TOB_WD2/Microscopy/20230331_Centrosome" + "/";
rawDir = root + "Crop_CVcrop" + "/";
binaryDir = root + "Binary_CentrosomesCEP192" + "/";
montageDir = root + "Binary_CentrosomesCEP192_Montages" + "/";
outDir = root + "CEP192_Volumes" + "/";
intensityDir = root + "3D_Intensity_CEP192" + "/";
...
for (i = 0; i < fileList.length; i++) 
	...
	saveAs("Results", intensityDir + file_naked_name + "_Pole_CEP192.csv");
	run("Clear Results");
	...
	setResult("Cell_ID", 0, file_naked_name);
	updateResults();
	saveAs("Results", outDir + file_naked_name + "_centrosomeCEP192_volume.csv");
