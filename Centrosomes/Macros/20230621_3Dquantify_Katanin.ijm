root = "/Volumes/TOB_WD2/Microscopy/20211206_PTM_MAPs" + "/";
rawDir = root + "Crop_Isotropic_CVcrop" + "/";
binaryDir = root + "Binary_CentrosomesKatanin" + "/";
montageDir = root + "Binary_CentrosomesKatanin_Montages" + "/";
outDir = root + "Katanin_Volumes" + "/";
intensityDir = root + "3D_Intensity_Katanin" + "/";
...
for (i = 0; i < fileList.length; i++) 
	...
	saveAs("Results", intensityDir + file_naked_name + "_Pole_Katanin.csv");
	run("Clear Results");
	...
	setResult("Cell_ID", 0, file_naked_name);
	updateResults();
	saveAs("Results", outDir + file_naked_name + "_centrosomeKatanin_volume.csv");
